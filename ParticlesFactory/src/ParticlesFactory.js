...
export class ParticlesFactory {
  ...
  this.#offscreenCtx.fillRect(0, 0, this.canvasEl.width, this.canvasEl.height);
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (this.lines?.draw) {
        ...
        if (!particle) return;
        particle.size = size;
        particle.draw(this.#offscreenCtx, this.particles.fillStyle, this.particles.opacity);
      }
    }
  ...
  #handleLinesAndCollision(particle, startIndex, len) {
    for (let j = startIndex + 1; j < len; j++) {
      ...
      this.#drawLines(this.#offscreenCtx, particle, otherParticle, distance);
  ...
  #updatePosition() {
    ...
    const canvasWidth = isFullScreen ? window.innerWidth : this.canvas.width;
    const canvasHeight = isFullScreen ? window.innerHeight : this.canvas.height;
